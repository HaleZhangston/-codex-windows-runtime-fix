...
这个仓库现在主要用于维护 **Codex Windows Runtime Fix**：一个针对 Windows 版 Codex Desktop 特定启动故障的修复工具。

## 它解决什么问题？

在部分 Windows 环境中，Codex Desktop 更新或冷启动后可能出现以下故障模式：
...
- 临时 runtime 可能已有 `node.exe`，但缺少 `node_repl.exe`；
- 极深层 Node.js 依赖文件可能因为 Windows 文件保护或超长路径问题没有被完整复制。

这个项目用于自动识别并修复 **这种特定的 `cua_node` runtime staging 不完整问题**。
...
4. 从用户电脑上已经安装好的 Codex 包读取 `cua_node` runtime；
...
完整代码、使用方法、已知限制、日志位置和安全说明请查看：

**[CodexWindowsRuntimeFix/README.md](CodexWindowsRuntimeFix/README.md)**

## Release

当前首个整理版本为 **v1.0.0**，Release 中提供 ZIP 和 SHA-256 校验文件。

## License

Codex Windows Runtime Fix 使用 MIT License，详见 [`CodexWindowsRuntimeFix/LICENSE`](CodexWindowsRuntimeFix/LICENSE)。
